...
[Hierarchy Keyring Cache Identifier formula](#hierarchy-keyring-cache-identifier-formula),
if two KMS Relationships for a Hierarchy Keyring are
...
##### Option 1 (Recommended): Append the keystore-id / partition-id to the [Hierarchy Keyring cache identifier formula](#hierarchy-keyring-cache-identifier-formula).
...
##### Option 2: Append unique numbers to the cache identifiers of different keyrings starting from 0 to N-1 where N is the number of keystores
This is hard to do for a multi-threaded system.
...
has branch-key-digest=SHA512(branch-key-id).
...
This is pending CryptoBR's guidance
...
Hashing the branch keys will mean that the blob
being hashed will be of uniform length.
TODO(Ritvik): Add more

##### Option 3: Don't change anything.
This would mean that the formula will remain incoherent.


#### Issue 3: Which hashing algorithm should be used in the Hierarchy Keyring cache identifier and the CachingCMM?
...
Adding prefixes will prevent any possible collisions between
...
However, SHA384 is not prone to [Length Extension attacks](https://en.wikipedia.org/wiki/Length_extension_attack), unlike SHA512, which is a plus for SHA384. But, cache identifiers don't fall into the category of length extension attacks, which come into the picture when an attacker tries to reconstruct signatures of a tampered message without knowing the original message. Since the cache identifiers have nothing to do with signing, we do NOT need to use SHA384.
