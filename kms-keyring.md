...
[encrypted data key](#data-structures.md) if the client supplier return a client.
...
If the input [encryption materials](#data-structures.md#encryption-materials) do not contain a plaintext data key
and this keyring does not have a [generator](#generator) defined,
OnEncrypt MUST not modify the [encryption materials](#data-structures.md#encryption-materials.md)
and MUST fail.
...
- `NumberOfBytes`: the [key derivation input length](#algorithm-suites.md#key-derivation-input-length)
  specified by the [algorithm suite](#algorithm-suites.md) included in the input [encryption materials](#data-structures.md#encryption-materials).
- `EncryptionContext`: the [encryption context](#data-structures.md#encryption-context) included in
   the input [encryption materials](#data-structures.md#encryption-materials).
...
modify the [encryption materials](#data-structures.md#encryption-materials) and MUST fail.
...
- set the plaintext data key on the [encryption materials](#data-structures.md#encryption-materials)
  as the response `Plaintext`.
- append a new [encrypted data key](#data-structures.md#encrypted-data-key) to the encrypted data key list
  in the [encryption materials](#data-structures.md#encryption-materials), constructed as follows:
  - the [ciphertext](#data-structures.md#ciphertext) is the response `CiphertextBlob`.
  - the [key provider id](#data-structures.md#key-provider-id) is "aws-kms".
...
- append a new [record](#data-structures.md#record) to the [keyring trace](#data-structures.md#keyring-trace)
  in the input [encryption materials](#data-structures.md#encryption-materials), constructed as follows:
  - The string field KeyNamespace contains "aws-kms".
  - The string field KeyName contains [generator](#generator).
  - The [flags](#data-structures.md$flags) field of this record includes exactly the following flags:
    - [GENERATED DATA KEY](#data-structures.md#supported-flags)
    - [ENCRYPTED DATA KEY](#data-structures.md#supported-flags)
    - [SIGNED ENCRYPTION CONTEXT](#data-structures.md#supported-flags)

Given a plaintext data key in the [encryption materials](#data-structures.md#encryption-materials),
...
- `PlaintextDataKey`: the plaintext data key in the [encryption materials](#data-structures.md#encryption-materials).
- `EncryptionContext`: the [encryption context](#data-structures.md#encryption-context) included in
  the input [encryption materials](#data-structures.md#encryption-materials).
- `GrantTokens`: this keyring's [grant tokens](#grant-tokens)
...
If all Encrypt calls succeed, OnEncrypt MUST output the modified [encryption materials](#data-structures.md#encryption-materials).
...
The set of [encrypted data keys](#data-structures.md#encrypted-data-keys) that OnDecrypt MUST attempt
...
[encrypted data key](#data-structures.md#encrypted-data-key) in the input encrypted data key list
with the following conditions, until it successfully decrypts one:

- the [key provider ID](#data-structures.md#key-provider-id) field has the value "aws-kms"

Otherwise, OnDecrypt MUST attempt to decrypt each input [encrypted data key](#data-structures.md#encrypted-data-key)
in the input encrypted data key list with the following conditions, until it successfully decrypts one:

- the [key provider ID](#data-structures.md#key-provider-id) field has the value "aws-kms"
- the [key provider info](#data-structures.md#key-provider-info) has a value equal to one of the
  ARNs in this keyring's [key IDs](#key-ids) or the [generator](#generator).

To attempt to decrypt a particular [encrypted data key](#data-structures.md#encrypted-data-key),
...
- `CiphertextBlob`: the [encrypted data key ciphertext](#data-structures.md#ciphertext).
- `EncryptionContext`: the [encryption context](#data-structures.md#encryption-context) included in
  the input [decryption materials](#data-structures.md#decryption-materials).
- `GrantTokens`: this keyring's [grant tokens](#grant-tokens)

If the call to [KMS Decrypt](#kms-decrypt) does not succeed OnDecrypt MUST continue and attempt to
decrypt the remaining [encrypted data keys](#data-structures.md#encrypted-data-keys).

If the `KeyId` field in the response from [KMS Decrypt](#kms-decrypt) does not have a value equal to
the [encrypted data key's key provider info](##data-structures.md#key-provider-info), then OnDecrypt
MUST fail.

If the call to [KMS Decrypt](#kms-decrypt) succeeds OnDecrypt MUST verify the following:

- verify that the `KeyId` field has a value equal to the [encrypted data key's key provider info](##data-structures.md#key-provider-info).
...
If any of the above are not true, OnDecrpyt MUST NOT update the [decryption materials](#data-structures.md#decryption-materials)
and MUST fail.

If the response is successfully verified, OnDecrypt MUST do the following with the response:

- set the plaintext data key on the [decryption materials](#data-structures.md#decryption-materials) as the response `Plaintext`.
- append a new [record](#data-structures.md#record) to the [keyring trace](#data-structures.md#keyring-trace)
  in the input [encryption materials](#data-structures.md#encryption-materials), constructed as follows:
  - The string field KeyNamespace contains "aws-kms".
  - The string field KeyName contains [generator](#generator).
  - The [flags](#data-structures.md$flags) field of this record includes exactly the following flags:
    ...
    - [VERIFIED ENCRYPTION CONTEXT](#data-structures.md#supported-flags)
- immediately return the modified [decryption materials](#data-structures.md#decryption-materials).

If OnDecrypt fails to successfully decrypt any [encrypted data key](#data-structures.md#encrypted-data-key),
then OnDecrypt MUST output the unmodified input [decryption materials](#data-structures.md#decryption-materials).
